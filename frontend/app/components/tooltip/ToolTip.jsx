...
class Card extends React.Component {
  ...
  checkWindowPosition(style, arrowStyle)
  {
    if (this.props.position === 'top' || this.props.position === 'bottom')
    {
      ...
      {
        let rightOffset = style.left + this.state.width - (window.outerWidth + window.scrollX);
        if (rightOffset > 0)
        {
          let originalLeft = style.left
          style.left = style.left - (this.state.width / 2) - this.margin
  ...
  handleMouseLeave() {
    ...
    if (this.props.onToolTipMouseLeave)
    {
      this.props.onToolTipMouseLeave();
    }
  }
  componentDidMount() {
    this.updateSize()
  ...
  componentWillReceiveProps(nextProps) {
    ...
    this.setState({transition: this.state.hover || this.props.active ? 'all' : 'opacity'}, () => {this.updateSize()})
  }
  updateSize() {
...
export default class ToolTip extends React.Component {
  ...
  renderPortal(props)
  {
    ...
    renderSubtreeIntoContainer(this, <Card parentEl={parentEl} onToolTipMouseLeave={this.props.onToolTipMouseLeave} {...other}/>, portalNodes[this.props.group].node)
  }
